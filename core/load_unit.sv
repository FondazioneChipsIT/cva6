...
#(
    ...
    case (state_q)
      IDLE: begin
        if (accept_req) begin
          ...
          translation_req_o = (dtlb_hit_i || !lsu_ctrl_i.is_speculative_load);
          // check if load is speculative and non idempotent, if it is then stall and wait for branch result
          if (!lsu_ctrl_i.is_speculative_load || (dtlb_hit_i && !paddr_ni)) begin
      ...
      WAIT_SPEC_LOAD: begin
        ...
        if (lsu_ctrl_i.is_speculative_load_miss) begin
  ...
  always_comb begin : rvalid_output
    ...
    if ((state_q == WAIT_SPEC_LOAD) && lsu_ctrl_i.is_speculative_load_miss && !req_port_i.data_rvalid && valid_i) begin
      trans_id_o = lsu_ctrl_i.trans_id;
      valid_o = 1'b1;
      ex_o.valid = 1'b0;
    end
